          ...
          ""stringValue"": ""{username}""
            ...
            using (UnityWebRequest req = new UnityWebRequest(url, "POST"))
            {
                ...
                if (resp.Contains("\"document\"") && resp.Contains($"\"{password}\"") && !resp.Contains("\"error\""))
                {
                    ...
                    int docIndex = resp.IndexOf("documents/childaccounts/");
                    if (docIndex != -1)
                    {
                        int startIndex = docIndex + "documents/childaccounts/".Length;
                        int endIndex = resp.IndexOfAny(new char[] { '"', ' ', '\n' }, startIndex);
                        if (endIndex != -1)
                        {
                            string childid = resp.Substring(startIndex, endIndex - startIndex);
                            Debug.Log($"[FirebaseAuth] Extracted childid: {childid}");
                            PlayerPrefs.SetString("firebase_childid", childid);
                        }
                    }
                }
